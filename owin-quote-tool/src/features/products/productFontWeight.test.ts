...
describe('font weight của feature Sản phẩm', () => {
  it('khai báo đúng bốn token theo FONT_WEIGHT_GUIDE', () => {
    expect(stylesheet).toContain('--product-weight-regular: 400;');
    expect(stylesheet).toContain('--product-weight-medium: 500;');
    expect(stylesheet).toContain('--product-weight-semibold: 600;');
    expect(stylesheet).toContain('--product-weight-bold: 700;');
  });
  ...
  it('mọi font-weight trong CSS đều tham chiếu token của feature', () => {
    ...
    declarations.forEach((value) => {
      expect(value).toMatch(/^var\(--product-weight-(regular|medium|semibold|bold)\)(?:\s*!important)?$/);
    });
  });
